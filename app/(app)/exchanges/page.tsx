...
function isMarketOpen(openTime: string, closingTime: string, timeZone: string, names: string) {
    ...
    if (names === 'Crypto') return { localOpenTime: '00:00', localCloseTime: '23:59', status: 'open' };

    let status;
    const currentGermanTime = DateTime.now().setZone('Europe/Berlin');

    if (names === 'Forex') {
        const localBreakStart = DateTime.fromObject({ hour: 16, minute: 0 }, { zone: 'America/New_York' });
        const breakStartGermany = localBreakStart.setZone('Europe/Berlin');
        const localBreakEnd = DateTime.fromObject({ hour: 17, minute: 0 }, { zone: 'America/New_York' });
    ...
    const localOpenTime = DateTime.fromObject({ hour: hours1, minute: minutes1 }, { zone: timeZone });
    const localClosingTime = DateTime.fromObject({ hour: hours2, minute: minutes2 }, { zone: timeZone });
    const openTimeGermany = localOpenTime.setZone('Europe/Berlin');
    const closingTimeGermany = localClosingTime.setZone('Europe/Berlin');

    function specialBreakTimes(h1: number, m1: number, h2: number, m2: number) {
        const localBreakStart = DateTime.fromObject({ hour: h1, minute: m1 }, { zone: timeZone });
        const lokalBreakEnd = DateTime.fromObject({ hour: h2, minute: m2 }, { zone: timeZone });
        const breakStartGermany = localBreakStart.setZone('Europe/Berlin');
        const breakEndGermany = lokalBreakEnd.setZone('Europe/Berlin');
        const current = DateTime.now().setZone('Europe/Berlin');
        if (current >= breakStartGermany && current < breakEndGermany) return { status: 'closed' };
        return false;
    }
    ...
    if (currentGermanTime >= openTimeGermany && currentGermanTime <= closingTimeGermany) {
        ...
        if (currentGermanTime.weekday === 6 || currentGermanTime.weekday === 7) status = 'closed';
        else status = 'open';
    } else {
        status = 'closed';
    }
