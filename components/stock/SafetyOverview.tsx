...
export default function SafetyOverview({ stockData }: { stockData: StockData }) {
   ...
   const totalStocks = stocksWithSafety.length;

   // Smart counting to avoid duplicates
   let topCount, bottomCount;

   if (totalStocks <= 3) {
      topCount = totalStocks;
      bottomCount = 0; // Don't show bottom performers
   } else if (totalStocks <= 6) {
      topCount = Math.ceil(totalStocks / 2);
      bottomCount = Math.floor(totalStocks / 2);
   } else {
      topCount = 3;
      bottomCount = 3;
   }
   ...
   const StockRow = ({ stock, index, isTop }: { stock: AddStockType & { safetyMargin: string; safetyNumeric: number }; index: number; isTop: boolean }) => (
      <div className="flex justify-between items-center py-2 px-3 hover:bg-gray-50 rounded transition-colors">
         ...
         <span className={`font-bold text-sm ${stock.safetyMargin === "N/A" ? "text-red-500" : "text-green-600"}`}>{stock.safetyMargin === "N/A" ? "N/A" : `${(parseFloat(stock.safetyMargin) * 100).toFixed(0)}%`}</span>
      </div>
   );
   ...
   return (
      <div className="bg-white rounded-xl p-5 shadow-md w-full max-w-sm">
         ...
         {/* Top Performers */}
         <div className={bottomCount > 0 ? "mb-5" : ""}>
         ...
         {/* Only show divider and bottom section if we have bottom performers */}
         {bottomCount > 0 && (
            <>
               {/* Divider */}
